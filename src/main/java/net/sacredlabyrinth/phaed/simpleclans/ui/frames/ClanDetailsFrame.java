...
public class ClanDetailsFrame extends SCFrame {
	...
	private void addVerify() {
		...
		if (!verified) {
			if (purchaseVerification) {
				lore.add(lang("gui.clandetails.verify.price.lore", getViewer(), CurrencyFormat.format(settings.getDouble(ECONOMY_VERIFICATION_PRICE))));
			}
	...
	private void addRegroup() {
		...
		if (price != 0) lore.add(lang("gui.clandetails.regroup.lore.price", getViewer(), CurrencyFormat.format(price)));
	...
	private void addHome() {
		...
		List<String> lore = new ArrayList<>();
		if (homePrice != 0) lore.add(lang("gui.clandetails.home.lore.teleport.price", getViewer(), CurrencyFormat.format(homePrice)));
		lore.add(lang("gui.clandetails.home.lore.teleport", getViewer()));
		if (setPrice != 0) lore.add(lang("gui.clandetails.home.lore.set.price", getViewer(), CurrencyFormat.format(setPrice)));
